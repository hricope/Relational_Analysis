...
We get the data that was exported from WoS and Scopus

```{r cargaDB}

 file <- "C:/Users/hrico/OneDrive - Universidad de Oviedo/1.1 Herramienta analisis bibliometrico/scopus_21052022.bib" 

file2 <- "C:/Users/hrico/OneDrive - Universidad de Oviedo/1.1 Herramienta analisis bibliometrico/WoS_21052022.bib"
...
We review and clean the merged dataframe. Human review is taken place

-   Delete all raw whithout author in the field AU (field Tag = AU)
-   Delete duplicated raws: "JUNG D;DORNER V;GLASER F;MORANA
    S"
-   fix wrong year na --> 2021 ("HENTZEN JK, NA, INT J BANK MARK)
                      --> 2022 (DUC K N D;SERMPINIS G;STASINAKIS C)

```{r cleanning}
noAU_Rows<-which(w_sDF$AU=="NA N")
w_sDF<-w_sDF[-c(noAU_Rows),]

dupJung<-which(w_sDF$AU=="JUNG D;DORNER V;GLASER F;MORANA S")
 w_sDF<-w_sDF[-c(222),] # duplicated

a<-which(is.na(w_sDF$PY))
w_sDF[198,24]<-2021
w_sDF[201,24]<-2022
```
...
el objetivo es homogeneizar las partes del data frame que obtenemos al
realizar un merge de las bases de datos WoS y Scopus. En el Tag *CR* la
estructura que le dan a la parte del texto que se refiere a los autores
(AU) es diferente:

*Scopus: AU= 'Apellido', 'Iniciales'.,* WoS: AU= 'Apellido'
'Iniciales'.,

Esta diferencia crea problemas despues al usar funciones como
**localCitations** o **histNetwork** en las que usa el tag *SR* para
realizar estos análisis. *SR* es una estructura resumen que utiliza como
pattern en busqeudas dentro de *CR*. *SR* caracteriza el autor (AU) con
la estructura de WoS, por lo que si utilizamos data frame fusionados
entre Scopus y WoS, el análisis que obtenemos no es completo.

El final de cada referencia es ";" en ambos casos.

**nota:** no he podido obtener unos resultados fiables. La
homogeniezacion del *AU* no aporta cambios. Sin embargo al modificar el
DB para que utilice la funcion histNetwork basada en scopus me da unos
resultados que no son coherentes. (analizar mejor como obtiene el
**localCitations**)
...
## exporta los data frame:

**dataframe to csv** con este formato no se puede usar luego en
biblioshiny()

```{r exportDF}
write.csv2(scopusDF,file="C:/Users/hrico/OneDrive - Universidad de Oviedo/1.1 Herramienta analisis bibliometrico/scopusDF_21052022.csv")

write.csv2(wosDF,file="C:/Users/hrico/OneDrive - Universidad de Oviedo/1.1 Herramienta analisis bibliometrico/WoSDF_21052022.csv")

write.csv2(w_sDF,file="C:/Users/hrico/OneDrive - Universidad de Oviedo/1.1 Herramienta analisis bibliometrico/w_sDF_21052022.csv")
```

**dataframe to excel** con este formato nos permite cargar el dataframe
en bilbioshiny() como un bibliometrix file y utilizar todos los análisis
ya preparados

```{r DF2Biblioshiny, include=FALSE}
# homogeniezado
write_xlsx(w_sDF_H,"C:/Users/hrico/OneDrive - Universidad de Oviedo/1.1 Herramienta analisis bibliometrico/RA_results.xlsx")

# sin homogenizar
write_xlsx(w_sDF,"C:/Users/hrico/OneDrive - Universidad de Oviedo/1.1 Herramienta analisis bibliometrico/RA_results2.xlsx")
```

## Análisis descriptivo

La función biblioanañysis devuelve un objeto de la clase "bibliometrix"
que es una lista con los siguientes componentes:
...
|                              | manuscripts                  |
+------------------------------+------------------------------+
| \* Authors                   | the authors' frequency       |
|                              | distribution                 |
+------------------------------+------------------------------+
| \* AuthorsFrac               | the authors' frequency       |
|                              | distribution                 |
|                              | (fractionalized)             |
+------------------------------+------------------------------+
| \* FirstAuthors              | corresponding author of each |
|                              | manuscript                   |
+------------------------------+------------------------------+
| \* nAUperPaper \| m          | the number of authors per    |
|                              | anuscript                    |
+------------------------------+------------------------------+
| \* Appearances \| a          | the number of author         |
|                              | ppearances                   |
+------------------------------+------------------------------+
| \* nAuthors                  | the number of authors        |
+------------------------------+------------------------------+
| \* AuMultiAuthoredArt        | the number of authors of     |
|                              | multi-authored articles      |
+------------------------------+------------------------------+
| \* MostCitedPapers           | the list of manuscripts      |
|                              | sorted by citations          |
+------------------------------+------------------------------+
| \* Years                     | publication year of each     |
|                              | manuscript                   |
+------------------------------+------------------------------+
| \* FirstAffiliation          | the affiliation of the       |
|                              | corresponding author         |
+------------------------------+------------------------------+
| \* Affiliations              | the frequency distribution   |
|                              | of affiliations (of all      |
|                              | co-authors                   |
+------------------------------+------------------------------+
| for each paper)              |                              |
+------------------------------+------------------------------+
| \* Aff_frac                  | the fractionalized frequency |
|                              | distribution of affiliations |
|                              | (of all co-authors for each  |
+------------------------------+------------------------------+
| \* CO                        | the affiliation country of   |
|                              | the corresponding author     |
+------------------------------+------------------------------+
| \* Countries                 | the affiliation countries'   |
|                              | frequency distribution       |
+------------------------------+------------------------------+
| \* CountryCollaboration      | the intra-country (SCP) and  |
|                              | inter-country (MCP)          |
|                              | collaboration indices        |
+------------------------------+------------------------------+
| \* TotalCitation             | the number of times each     |
|                              | manuscript has been cited    |
+------------------------------+------------------------------+
| \* TCperYear                 | the yearly average number of |
|                              | times each manuscript has    |
|                              | been cited                   |
+------------------------------+------------------------------+
| \* Sources                   | the frequency distribution   |
|                              | of sources (journals, books, |
|                              | etc.)                        |
+------------------------------+------------------------------+
| \* DE                        | the frequency distribution   |
|                              | of authors' keywords         |
+------------------------------+------------------------------+
| \* ID                        | the frequency distribution   |
|                              | of keywords associated to    |
|                              | the manuscript by SCOPUS and |
+------------------------------+------------------------------+

```{r descriptive analysis, include=FALSE}
# resultados con los datos homoegeniazados

 results_21052022 <- biblioAnalysis(w_sDF_H, sep = ";")
```
...
```{r}
options(width=100)
S <- summary(object = results_21052022, k = 10, pause = FALSE)


plot(x = results_21052022, k = 10, pause = FALSE)
```
...
### Citas
```{r citas}
CR <- citations(w_sDF_H, field = "article", sep = ";")
...
el objetivo es realizar un análisis desriptivo de las keywords que
representan la investigacion sobre RA en nuestra collección de
documentos. En el data frame que estamos utilizando se encuentran en los
siguientes campos:

-   TI: Título deñ documento
-   DE: Keywords del autor
-   ID: Keyword plus, algoritmo realizado por bibliometrix() para
    encontrar las keywords presentes en los títulos de las referencias
    citadas en los articulos del autor y no necesariamente en el título
    del artículo del autor.
-   AB: Abstracts

#### cleaning de palabras no relevantes

al realizar la query inicial basada en todos los términos "robo-advi*",
eliminamos todas las combinaciones de palabras con esos dos términos
"Robo" y "Advi*"

##### descripcion función termExtractio()
...
-   @param **stemming** is logical. If TRUE the Porter Stemming
    algorithm is applied to all extracted terms. The default is
    \code{stemming = FALSE}.

-   @param **language** is a character. It is the language of textual
    contents ("english", "german","italian","french","spanish"). The
    default is \code{language="english"}.

-   @param **remove.numbers** is logical. If TRUE all numbers are
    deleted from the documents before term extraction. The default is
    \code{remove.numbers = TRUE}.

-   @param **remove.terms** is a character vector. It contains a list of
    additional terms to delete from the documents before term
    extraction. The default is \code{remove.terms = NULL}.

-   @param **keep.terms** is a character vector. It contains a list of
    compound words "formed by two or more terms" to keep in their
    original form in the term extraction process. The default is
    \code{keep.terms = NULL}.

-   @param **synonyms** is a character vector. Each element contains a
    list of synonyms, separated by ";", that will be merged into a
    single term (the first word contained in the vector element). The
    default is \code{synonyms = NULL}.

-   @param **verbose** is logical. If TRUE the function prints the most
    frequent terms extracted from documents. The default is
    \code{verbose=TRUE}.

-   @return the bibliometric data frame with a new column containing
    terms about the field tag indicated in the argument \code{Field}.

## creacion ra_STOPWORDS

### 1ª extracción bruta de keywords

hacemos una primera extracción de las keywords de los tags TI,DE,AB para con esta lista de palabras construir las stopwords y el diccionario de sinónimos de forma manual.
...
# creo un Df para trabajar con el

myDF <- w_sDF_H

#cargamos las stopwords basicas del ingles
...
# extracción de los terminos para el tag Ab y diferentes ngrams. lo hacemos para ngrams 1 to 3
...
#los ponemos en un unico DF para analizarlos en una unica table
Term_Raw <- rbind(DE_TermsDF,AB_Rawn1DF,AB_Rawn2DF,AB_Rawn3DF) # 12/06/22: 14377 Keywords

Term_Raw2<- rbind(DE_TermsDF,AB_Rawn1DF,AB_Rawn2DF) # 12/06/22: 11091 Keyword

# exportamos a un csv para realizar el análisis manual de los términos y completar el RA Stopwords
...
palabras que no queremos que se incluyan, se sumarian a la lista
generica stopwords que usa la función por default
...
RA_Stopwords <- c(RA_Stopwords$StopWords) # paso a un vector de caracteres
# pasamos a minusculass
RA_Stopwords <- tolower(RA_Stopwords)

# fusionAMOS LAS DOS LISTAS DE PALABRAS

remove.terms <- c(RA_Stopwords,sw)

# pasamos a mayusculas ya que todo el corpus esta en mayusculas
...
la primera palabra de cada fila de cadena de texto la toma como la
principal y el resto de la fila son sus sinónimos.

# cargamos los sinonimos

cargamos las agrupaciones de palabas en conceptos relevantes. Creamos un
data frame con estas agrupaciones la última columna de este diccionario
de conceptos son las stopwords especificas del tema

```{r sinonimos carga, include=FALSE}
library(readxl)
...
# pasamos todas las filas a un unico vector de caracteres

construir un vector con todos los campos sinonimos desde un data frame
con un bucle cargamos todos los campos conceptuales (columnas) y los
vamos a agragando a un vector de caracteres que luego utilizaremos como
sinonimos para representar los graficos o agrupacion de palabras

```{r Sinonimos}


nCampos <- ncol(RA_Sinonimos) # guardo el numero de campos que tengo


#creamos el vector de caracteres donde vamos a almacenar
synonyms_RA <- character()

for(i in 1:nCampos){
  
  # obtengo el indice donde estan los NA
  ...
  if(length(m)==0){
    ...
    # convierto en una lista la columna de char
    z <- c(RA_Sinonimos[,i][1:min(m)-1,]) 
  }  
  # convierto en character la lista anterior
  z <- unlist(z) 
  # collapso todo el vector de string en uno solo separado por ";"
...
Para ver la relavancia del stopwords creado vamos a comparar la
frecuencia de los terminos sin y con stopwords.Se realiza sobre un objeto bibliometrix

```{r run analysis_AB }
# AB --run termExtraction()

# extracción de los terminos sin stopwords RA. lo hacemos para ngrams 1 to 2.
...
# 25/06/22: hacemos el análisis solo para ngram 1 to 2, ngram = 3 no aporta despues de hacer la revisión de las palabras extraidas


#los ponemos en un unico DF para representarlos en un wordcloud
AB_Raw <- rbind(AB_Rawn1DF,AB_Rawn2DF)

# extraemos los terminos sin agrupacion por sinonimos. lo utilizamos luego para saber las repeticiones de cada termino sin agrupar
...
#los agrupamos todos en un unico data frame. lo utilizaremos en el grafico de circulos para mostrar la agrupacion por sinonimos y el peso de cada agrupación
AB_terms.noSin <- rbind(AB_TermsRawn1DF,AB_TermsRawn2DF)

#expotamos ambos sets de palabras, lo utilizamos en RA_sinomimos para preparar data4bubble y bubble vertices 
...
# extraemos los terminos agrupados por sinonimos
...
#expotamos ambos sets de palabras
...
analizamos la relevancia de hacer el análisis con stopword y sinonimos. Nos centramos en las keywords de los abstracts(AB)
...
# me quedo con las x palabras mas relevantes


# las top palabrass sin sinonimos ni RA stopwords
...
# las top palabrass sin sinonimos 
...
# las top palabrass con sinonimos y RA stopwords
...
# representamos las palabras por debajo de los top
AB_Bottom_Term <- AB_TermsDF[AB_TermsDF$Freq<25 & AB_TermsDF$Freq>10,]

wordcloud2(AB_Bottom_Term,size = 0.08, minSize = 0.2, gridSize =  0,
    fontFamily = 'Segoe UI', fontWeight = 'bold',
    color = 'random-dark', backgroundColor = "white",
    minRotation = -pi/4, maxRotation = pi/4, shuffle = TRUE,
    rotateRatio = 0.4, shape = 'circle', ellipticity = 0.65,
...
## descripcion del diccionario de sinónimos.

en este apartado mostramos las agrupacines que hemos realizado en el
diccionario de sinonimos y el peso de cada agrupacion en función de la
repetición de los terminos que las componen.

```{r descripcion sinonimos & bubble graph}
...
# para completar estos datos he utilizado el output AB_Terms, que ya ha sido filtrado con stopwords y sinonimos
...
### definimos la relacion entre los cluster y los sinonimos
